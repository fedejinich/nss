...
FLOW_NODES: list[FlowNodeSpec] = [
    FlowNodeSpec(
        ...
        "server_tx",
        "Normalizes search text before serialization.",
    ),
    FlowNodeSpec(
        "server_file_search",
        "Server::FileSearch(QString, QString)",
        "server_tx",
        "Builds and sends search message to the server.",
    ),
    FlowNodeSpec(
        "server_send_message",
        "Server::SendMessage(MemStream&, bool)",
        "server_tx",
        "Serializes MemStream into the server socket.",
    ),
    FlowNodeSpec(
    ...
    FlowNodeSpec(
        ...
        "peer_tx",
        "Builds the initial download request sent to a peer.",
    ),
    FlowNodeSpec(
        "peer_send_message",
        "PeerMessenger::SendMessage(QTcpSocket*, MemStream&, bool)",
        "peer_tx",
        "Serializes messages on the peer socket.",
    ),
    FlowNodeSpec(
        "peer_handle_message",
        "PeerMessenger::HandleMessage(QTcpSocket*, MemStream)",
        "peer_rx",
        "Dispatches incoming peer messages to transfer handlers.",
    ),
    FlowNodeSpec(
        ...
        "transfer_ctrl",
        "Creates local queue entry and starts file connection flow.",
    ),
    FlowNodeSpec(
        ...
        "transfer_ctrl",
        "Negotiates transfer request/response with peer.",
    ),
    FlowNodeSpec(
        "download_read_socket",
        "DownloadTask::readSocket()",
        "download_rx",
        "Consumes socket bytes and writes destination file.",
    ),
    FlowNodeSpec(
        "upload_write_socket",
        "UploadTask::writeToSocket()",
        "upload_tx",
        "Streams local bytes to the remote peer.",
    ),
]


FLOW_EDGES: list[FlowEdgeSpec] = [
    FlowEdgeSpec("server_file_search", "server_prepare_search", "file_search prepares terms before frame build"),
    FlowEdgeSpec("server_file_search", "server_send_message", "file_search ends in send_message"),
    FlowEdgeSpec("server_send_message", "server_handle_message", "request/response over server socket"),
    FlowEdgeSpec("peer_queue_download", "transfer_on_queue_download", "queue_download creates transfer queue entry"),
    FlowEdgeSpec("transfer_on_queue_download", "peer_send_message", "download queue emits request to peer"),
    FlowEdgeSpec("peer_send_message", "peer_handle_message", "peer response returns through handle_message"),
    FlowEdgeSpec("peer_handle_message", "transfer_on_file_request", "transfer_request dispatch"),
    FlowEdgeSpec("transfer_on_file_request", "download_read_socket", "accepted request starts download"),
    FlowEdgeSpec("transfer_on_file_request", "upload_write_socket", "accepted request may start upload"),
]
