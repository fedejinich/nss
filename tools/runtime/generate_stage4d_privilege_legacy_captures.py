...
CODE_SM_PRIVILEGED_LIST = 69
CODE_SM_GET_RECOMMENDED_USERS = 110
CODE_SM_GET_TERM_RECOMMENDATIONS = 111
CODE_SM_GET_RECOMMENDATION_USERS = 112
CODE_SM_BAN_USER = 132

CODE_PM_INVITE_USER_TO_ROOM = 10
CODE_PM_CANCELLED_QUEUED_TRANSFER = 14
CODE_PM_MOVE_DOWNLOAD_TO_TOP = 34
CODE_PM_EXACT_FILE_SEARCH_REQUEST = 47
CODE_PM_QUEUED_DOWNLOADS = 48
...
def write_run(
    *,
    ...
    manifest = {
        ...
        "notes": "Stage 4D runtime capture generated for privilege/messaging gaps and peer legacy cleanup.",
    }
...
def encode_queued_downloads_payload(paths: list[str]) -> bytes:
    payload = u32(len(paths))
    for path in paths:
        payload += s(path)
    return payload
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate Stage 4D runtime captures (privilege/messaging + peer legacy)"
    )
    ...
    parser.add_argument("--term", default="idm")
    parser.add_argument("--room", default="nicotine")
    parser.add_argument("--virtual-path", default="Music\\\\legacy-track.flac")
    ...
    def scenario_login_privilege_messaging() -> tuple[list[bytes], list[dict[str, object]]]:
        frames = send_server_sequence(
            server,
            [
                login_frame,
                frame(CODE_SM_PRIVILEGED_LIST, b""),
                frame(CODE_SM_GET_RECOMMENDED_USERS, b""),
                frame(CODE_SM_GET_TERM_RECOMMENDATIONS, s(args.term)),
                frame(CODE_SM_GET_RECOMMENDATION_USERS, s(args.term)),
                frame(CODE_SM_BAN_USER, s(target_user)),
            ],
        ...
        events = [
            {
                "ts": now_iso(),
                "event": "privilege.messaging.attempts",
                "server": server,
    ...
    def scenario_peer_legacy_local() -> tuple[list[bytes], list[dict[str, object]]]:
        frames = [
            frame(CODE_PM_INVITE_USER_TO_ROOM, s(args.room)),
            frame(CODE_PM_CANCELLED_QUEUED_TRANSFER, s(args.virtual_path)),
            frame(CODE_PM_MOVE_DOWNLOAD_TO_TOP, s(args.virtual_path)),
            frame(
                CODE_PM_QUEUED_DOWNLOADS,
                encode_queued_downloads_payload([args.virtual_path, "Music\\\\legacy-b.flac"]),
            ),
            frame(CODE_PM_EXACT_FILE_SEARCH_REQUEST, u32(777) + s(args.virtual_path)),
            frame(CODE_PM_INDIRECT_FILE_SEARCH_REQUEST, s("legacy-track.flac")),
        ]
        events = [
            {
                "ts": now_iso(),
                "event": "peer.legacy.local",
    ...
    scenarios: list[
        ...
        (
            ...
            scenario_login_privilege_messaging,
        ),
        (
            "peer-legacy-local",
            "peer-legacy-local",
            "runtime_peer_local_deterministic",
            scenario_peer_legacy_local,
        ),
    ]
