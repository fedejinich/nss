...
CODE_SM_SAY_CHATROOM = 13
CODE_SM_JOIN_ROOM = 14
CODE_SM_LEAVE_ROOM = 15
CODE_SM_ROOM_LIST = 64
CODE_SM_ROOM_MEMBERS = 133
CODE_SM_ROOM_OPERATORS = 148
...
def write_run(
    *,
    ...
    manifest = {
        ...
        "notes": "Runtime-authenticated room/presence capture generated for S3B.",
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate authenticated Stage 3B room/presence runtime captures"
    )
    ...
    def scenario_login_room_list() -> tuple[list[bytes], list[dict[str, object]]]:
        frames = send_server_sequence(
            server,
            [
                login_frame,
                frame(CODE_SM_ROOM_LIST, b""),
            ],
            wait_after_each_s=0.8,
            final_wait_s=2.0,
        )
        events = [
            {
    ...
    def scenario_login_join_room_presence() -> tuple[list[bytes], list[dict[str, object]]]:
        probe_message = "NeoSoulSeek S3B runtime room probe"
        frames = send_server_sequence(
            server,
            [
                login_frame,
                frame(CODE_SM_JOIN_ROOM, s(room)),
                frame(CODE_SM_ROOM_MEMBERS, s(room)),
                frame(CODE_SM_ROOM_OPERATORS, s(room)),
                frame(CODE_SM_SAY_CHATROOM, s(room) + s(probe_message)),
            ],
            wait_after_each_s=1.1,
            final_wait_s=8.0,
        )
        events = [
            {
                "ts": now_iso(),
                "event": "room.join_presence",
                "server": server,
                "room": room,
    ...
    def scenario_login_leave_room() -> tuple[list[bytes], list[dict[str, object]]]:
        frames = send_server_sequence(
            server,
            [
                login_frame,
                frame(CODE_SM_JOIN_ROOM, s(room)),
                frame(CODE_SM_LEAVE_ROOM, s(room)),
            ],
    ...
    scenarios: list[tuple[str, str, Callable[[], tuple[list[bytes], list[dict[str, object]]]]]] = [
        ("login-room-list", "login-room-list", scenario_login_room_list),
        ("login-join-room-presence", "login-join-room-presence", scenario_login_join_room_presence),
        ("login-leave-room", "login-leave-room", scenario_login_leave_room),
    ]
