...
CODE_SM_SET_STATUS = 28
CODE_SM_HEARTBEAT = 32
CODE_SM_DNET_RESET = 130
...
def write_run(
    *,
    ...
    manifest = {
        ...
        "notes": (
            "Stage 5H runtime capture for typed system-control payloads "
            "(SM_SET_STATUS, SM_HEARTBEAT, SM_DNET_RESET)."
        ),
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate Stage 5H runtime capture for system-control payloads (28, 32, 130)."
    )
    ...
    parser.add_argument("--heartbeat-sequence", type=int, default=1)
    parser.add_argument("--dnet-reset-reason", type=int, default=1)
    parser.add_argument("--run-id", default="login-system-control")
    ...
    frames = send_server_sequence(
        ...
        [
            login_frame,
            frame(CODE_SM_SET_STATUS, u32(args.status)),
            frame(CODE_SM_HEARTBEAT, u32(args.heartbeat_sequence)),
            frame(CODE_SM_DNET_RESET, u32(args.dnet_reset_reason)),
        ],
    )
    ...
    events = [
        {
            "ts": now_iso(),
            "event": "s5h.system-control.capture",
            "server": server,
            "status": args.status,
            "heartbeat_sequence": args.heartbeat_sequence,
            "dnet_reset_reason": args.dnet_reset_reason,
