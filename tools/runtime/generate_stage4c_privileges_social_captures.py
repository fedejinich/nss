...
CODE_SM_IGNORE_USER = 11
CODE_SM_UNIGNORE_USER = 12
CODE_SM_GET_OWN_PRIVILEGES_STATUS = 92
CODE_SM_GET_USER_PRIVILEGES_STATUS = 122
CODE_SM_GIVE_PRIVILEGE = 123
CODE_SM_INFORM_USER_OF_PRIVILEGES = 124
CODE_SM_INFORM_USER_OF_PRIVILEGES_ACK = 125

CODE_PM_GET_SHARED_FILES_IN_FOLDER = 36
CODE_PM_SHARED_FILES_IN_FOLDER = 37
...
def write_run(
    *,
    ...
    manifest = {
        ...
        "notes": "Stage 4C runtime capture generated for privileges/social control and peer folder coverage.",
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate Stage 4C runtime captures (privileges/social + peer folder)"
    )
    ...
    parser.add_argument("--token", type=int, default=4242)
    parser.add_argument("--directory", default="Music\\\\Albums")
    ...
    def scenario_login_privileges_social() -> tuple[list[bytes], list[dict[str, object]]]:
        frames = send_server_sequence(
            server,
            [
                login_frame,
                frame(CODE_SM_IGNORE_USER, s(target_user)),
                frame(CODE_SM_UNIGNORE_USER, s(target_user)),
                frame(CODE_SM_GET_OWN_PRIVILEGES_STATUS, b""),
                frame(CODE_SM_GET_USER_PRIVILEGES_STATUS, s(target_user)),
                frame(CODE_SM_GIVE_PRIVILEGE, s(target_user) + u32(args.days)),
                frame(CODE_SM_INFORM_USER_OF_PRIVILEGES, u32(args.token) + s(target_user)),
                frame(CODE_SM_INFORM_USER_OF_PRIVILEGES_ACK, u32(args.token)),
            ],
        ...
        events = [
            {
                "ts": now_iso(),
                "event": "privileges.social.attempts",
                "server": server,
    ...
    def scenario_peer_folder_local() -> tuple[list[bytes], list[dict[str, object]]]:
        listing_plain = b"SongA.flac\nSongB.mp3\n"
        compressed = zlib.compress(listing_plain)
        frames = [
            frame(CODE_PM_GET_SHARED_FILES_IN_FOLDER, s(args.directory)),
            frame(CODE_PM_SHARED_FILES_IN_FOLDER, s(args.directory) + compressed),
        ]
        events = [
            {
                "ts": now_iso(),
                "event": "peer.folder.local",
    ...
    scenarios: list[
        ...
        (
            ...
            scenario_login_privileges_social,
        ),
        (
            "peer-folder-local",
            "peer-folder-local",
            "runtime_peer_local_deterministic",
            scenario_peer_folder_local,
        ),
    ]
