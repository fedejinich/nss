...
CODE_SM_ADD_CHATROOM = 10
CODE_SM_ADD_LIKE_TERM = 51
CODE_SM_REMOVE_LIKE_TERM = 52
...
def write_run(
    *,
    ...
    manifest = {
        ...
        "notes": (
            "Stage 5C runtime capture for typed room/term control messages "
            "(SM_ADD_CHATROOM, SM_ADD_LIKE_TERM, SM_REMOVE_LIKE_TERM)."
        ),
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Generate Stage 5C runtime capture for room/term control "
            "(SM_ADD_CHATROOM, SM_ADD_LIKE_TERM, SM_REMOVE_LIKE_TERM)."
        )
    )
    ...
    parser.add_argument("--run-id", default="login-room-term-control")
    ...
    frames = send_server_sequence(
        ...
        [
            login_frame,
            frame(CODE_SM_ADD_CHATROOM, s(args.room)),
            frame(CODE_SM_ADD_LIKE_TERM, s(args.term)),
            frame(CODE_SM_REMOVE_LIKE_TERM, s(args.term)),
        ],
    )
    ...
    events = [
        {
            "ts": now_iso(),
            "event": "s5c.room-term-control.capture",
            "server": server,
            "room": args.room,
            "term": args.term,
    ...
    run_dir = write_run(
        repo_root=REPO_ROOT,
        run_id=args.run_id,
        scenario="login-room-term-control",
