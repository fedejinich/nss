...
CODE_SM_REMOVE_OWN_ROOM_MEMBERSHIP = 136
CODE_SM_GIVE_UP_ROOM = 137
CODE_SM_ADD_ROOM_MEMBERSHIP = 139
CODE_SM_REMOVE_ROOM_MEMBERSHIP = 140
CODE_SM_ADD_ROOM_OPERATORSHIP = 145
...
def write_run(
    *,
    ...
    manifest = {
        ...
        "notes": (
            "Stage 5F runtime capture for typed private-room membership/ownership controls "
            "(SM_REMOVE_OWN_ROOM_MEMBERSHIP, SM_GIVE_UP_ROOM, "
            "SM_ADD_ROOM_MEMBERSHIP, SM_REMOVE_ROOM_MEMBERSHIP, "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Generate Stage 5F runtime capture for private-room membership/ownership "
            "controls (136, 137, 139, 140, 145)."
        )
    )
    ...
    parser.add_argument("--run-id", default="login-private-room-membership-control")
    ...
    frames = send_server_sequence(
        ...
        [
            login_frame,
            frame(CODE_SM_REMOVE_OWN_ROOM_MEMBERSHIP, s(args.room)),
            frame(CODE_SM_GIVE_UP_ROOM, s(args.room)),
            frame(CODE_SM_ADD_ROOM_MEMBERSHIP, s(args.room)),
            frame(CODE_SM_REMOVE_ROOM_MEMBERSHIP, s(args.room)),
            frame(CODE_SM_ADD_ROOM_OPERATORSHIP, s(args.room)),
        ],
    )
    ...
    events = [
        {
            "ts": now_iso(),
            "event": "s5f.private-room-membership-control.capture",
            "server": server,
            "room": args.room,
    ...
    run_dir = write_run(
        repo_root=REPO_ROOT,
        run_id=args.run_id,
        scenario="login-private-room-membership-control",
